...
from ..models import Group, GroupMembership, Account, Board
from ..auth import require_admin, require_user, optional_user, require_board_owner, get_default_board_id, check_board_visible, get_user_by_uid
...
def list_groups(search: str = "", board_id: int = None, db: Session = Depends(get_db), user: dict | None = Depends(optional_user)):
    query = db.query(Group)
    if board_id:
        query = query.filter(Group.board_id == board_id)
        check_board_visible(db, board_id, user)
    elif user is None:
        ...
        if u:
            ...
            query = query.filter(Group.board_id.in_(own_ids))
        else:
            return []
    if search:
        like = f"%{search}%"
...
def get_group(group_id: int, db: Session = Depends(get_db), user: dict | None = Depends(optional_user)):
    g = db.query(Group).get(group_id)
    if not g:
        raise HTTPException(status_code=404, detail="分组不存在")
    if g.board_id:
        check_board_visible(db, g.board_id, user)
...
def update_group(group_id: int, data: schemas.GroupUpdate, db: Session = Depends(get_db), user: dict = Depends(require_user)):
    ...
    if not g:
        raise HTTPException(status_code=404, detail="分组不存在")
    if g.board_id:
        require_board_owner(db, g.board_id, user)
    changed = False
    for field, val in data.model_dump(exclude_unset=True).items():
        if field == "tags" and val is not None:
            setattr(g, field, json.dumps(val, ensure_ascii=False)); changed = True
        elif val is not None:
            setattr(g, field, val); changed = True
    if changed:
        audit_log(db, user["username"], "update", "group", group_id, {"changed_fields": list(data.model_dump(exclude_unset=True).keys())})
    db.commit(); db.refresh(g)
    return _group_to_detail(g)
...
def list_group_members(group_id: int, db: Session = Depends(get_db), user: dict | None = Depends(optional_user)):
    g = db.query(Group).get(group_id)
    if not g:
        raise HTTPException(status_code=404, detail="分组不存在")
    if g.board_id:
...
def update_membership(group_id: int, membership_id: int, data: schemas.MembershipUpdate, db: Session = Depends(get_db), user: dict = Depends(require_user)):
    ...
    changed = False
    for field, val in data.model_dump(exclude_unset=True).items():
        setattr(m, field, val); changed = True
    if changed:
        audit_log(db, user["username"], "update", "membership", membership_id, {"changed_fields": list(data.model_dump(exclude_unset=True).keys())})
    db.commit(); db.refresh(m)
    return schemas.MembershipBrief(
        id=m.id, account_id=m.account_id, group_id=m.group_id,
        group_nickname=m.group_nickname, joined_at=m.joined_at,
