...
def _account_to_detail(a: Account) -> schemas.AccountDetail:
    return schemas.AccountDetail(
        ...
        current_nickname=a.current_nickname,
        current_avatar=a.current_avatar,
        nickname_histories=[
...
def list_accounts(person_id: int, db: Session = Depends(get_db)):
    ...
    accounts = (
        db.query(Account)
        .options(joinedload(Account.nickname_histories))
...
def list_memberships(person_id: int, account_id: int, db: Session = Depends(get_db)):
    ...
    memberships = (
        ...
        .order_by(GroupMembership.is_pinned.desc(), GroupMembership.id)
        .all()
    )
    return [
        schemas.MembershipBrief(
            ...
            group=schemas.GroupBrief(
                id=m.group.id,
                group_number=m.group.group_number,
                group_name=m.group.group_name,
                remark=m.group.remark,
                tags=json.loads(m.group.tags or "[]"),
